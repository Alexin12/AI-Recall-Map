...
- 2026-07-21 (M3, e2e): Adopt Playwright for frontend UI end-to-end tests — Chromium-only, co-located in `frontend/` (`pnpm test:e2e`), selectors by `data-testid`/role/text and never CSS or position, starting with one core happy-path spec mirroring `backend/tests/test_e2e_core_loop.py`. Why: catch core-flow regressions in a real browser without pinning tests to a still-changing UI. [ADR-0003](docs/adr/0003-playwright-e2e-with-llm-fake-toggle.md)
- 2026-07-21 (M3, e2e): Browser e2e gets deterministic LLM output via a backend `LLM_FAKE=1` env toggle that stubs all five seams in `backend/app/llm.py` (returning canned Pydantic objects, defined inline, default off); pytest keeps monkeypatch, which cannot reach a separate live-server process. Why: a live server can't be monkeypatched and LLM nondeterminism would make e2e flaky. [ADR-0003](docs/adr/0003-playwright-e2e-with-llm-fake-toggle.md)
- 2026-07-21 (M3, e2e): e2e is orchestrated by Playwright `webServer` (starts backend with `LLM_FAKE=1` + frontend; Supabase started separately and left running), needs no data cleanup (fresh isolated demo user per run via the app's auto-signup + RLS), is gated into no-mistakes `commands.test` (pytest + e2e, Playwright evidence stored on the PR), and runs in a new GitHub Actions workflow using `supabase start` for the full auth stack — built local-green-first, CI last. Why: wire e2e into the existing gate and CI without per-test DB teardown. [ADR-0003](docs/adr/0003-playwright-e2e-with-llm-fake-toggle.md)
